...
function (m::PointProjection)(activations, rigids::Rigid)
    ...
    H = m.no_heads
    P = m.num_points
    raw = reshape(raw, size(raw, 1), size(raw, 2), P, H, 3)
    points_local = permutedims(raw, (1, 2, 4, 3, 5))
...
function (m::InvariantPointAttention)(s, z, r::Rigid, mask)
    ...
    L = size(q_bhlc, 3)
    C = size(q_bhlc, 4)
    q3 = permutedims(reshape(q_bhlc, B * H, L, C), (2, 3, 1)) # (L, C, B*H)
    ...
    o_pt = permutedims(o_pt, (1, 2, 4, 3, 5)) # (B, L, P, H, 3)
...
@concrete struct StructureModuleJL <: Onion.Layer
    inner::StructureModule
end

@layer StructureModuleJL

function StructureModuleJL(; cfg::StructureModuleConfig=StructureModuleConfig())
    return StructureModuleJL(StructureModule(cfg=cfg))
end


function (m::StructureModuleJL)(evoformer_output_dict, aatype, mask=nothing)
    # Inputs are feature-first, batch-last.
    s = evoformer_output_dict[:single] # (C_s, L, B)
    z = evoformer_output_dict[:pair]   # (C_z, L, L, B)

    s_fl = permutedims(s, (3, 2, 1))   # (B, L, C_s)
    z_fl = permutedims(z, (4, 2, 3, 1)) # (B, L, L, C_z)
    mask_fl = mask === nothing ? nothing : permutedims(mask, (2, 1))
    aatype_fl = ndims(aatype) == 2 ? permutedims(aatype, (2, 1)) : aatype

    out_fl = m.inner(Dict(:single => s_fl, :pair => z_fl), aatype_fl, mask_fl)

    out = Dict{Symbol,Any}()
    out[:single] = permutedims(out_fl[:single], (3, 2, 1))
    out[:frames] = permutedims(out_fl[:frames], (1, 4, 3, 2))
    out[:sidechain_frames] = permutedims(out_fl[:sidechain_frames], (1, 4, 5, 6, 3, 2))
    out[:unnormalized_angles] = permutedims(out_fl[:unnormalized_angles], (1, 4, 5, 3, 2))
    out[:angles] = permutedims(out_fl[:angles], (1, 4, 5, 3, 2))
    out[:positions] = permutedims(out_fl[:positions], (1, 5, 4, 3, 2))
    out[:states] = permutedims(out_fl[:states], (1, 4, 3, 2))
...
@concrete struct PointProjectionJL <: Onion.Layer
    linear
    num_points::Int
    no_heads::Int
end

@layer PointProjectionJL

function PointProjectionJL(c_hidden::Int, num_points::Int, no_heads::Int)
    linear = LinearFirst(c_hidden, no_heads * 3 * num_points)
    return PointProjectionJL(linear, num_points, no_heads)
end

function (m::PointProjectionJL)(activations, rigids::RigidJL)
    ...
    points_global = apply_rigid_jl(rigids, points_local)
    return points_global
end

@concrete struct InvariantPointAttentionJL <: Onion.Layer
    c_s::Int
    c_z::Int
    c_hidden::Int
    no_heads::Int
...
@layer InvariantPointAttentionJL

function InvariantPointAttentionJL(c_s::Int, c_z::Int, c_hidden::Int, no_heads::Int, no_qk_points::Int, no_v_points::Int; inf::Real=1e5, eps::Real=1e-8)
    linear_q = LinearFirst(c_s, c_hidden * no_heads)
    linear_q_points = PointProjectionJL(c_s, no_qk_points, no_heads)
    linear_kv = LinearFirst(c_s, 2 * c_hidden * no_heads)
    linear_kv_points = PointProjectionJL(c_s, no_qk_points + no_v_points, no_heads)
    linear_b = LinearFirst(c_z, no_heads)
    head_weights = fill(0.54132485f0, no_heads)
    linear_out = LinearFirst(no_heads * (c_z + c_hidden + no_v_points * 4), c_s)
    return InvariantPointAttentionJL(
        c_s,
        c_z,
        c_hidden,
        no_heads,
...
function (m::InvariantPointAttentionJL)(s, z, r::RigidJL, mask)
    ...
    o_pt = invert_apply_rigid_jl(r, permutedims(o_pt, (5, 4, 3, 2, 1))) # (3, P, H, L, B)
...
@concrete struct BackboneUpdateJL <: Onion.Layer
    linear
end

@layer BackboneUpdateJL

function BackboneUpdateJL(c_s::Int)
    return BackboneUpdateJL(LinearFirst(c_s, 6))
end

(m::BackboneUpdateJL)(s) = m.linear(s)

@concrete struct StructureModuleTransitionLayerJL <: Onion.Layer
    linear_1
    linear_2
    linear_3
end

@layer StructureModuleTransitionLayerJL

function StructureModuleTransitionLayerJL(c::Int)
    linear_1 = LinearFirst(c, c)
    linear_2 = LinearFirst(c, c)
    linear_3 = LinearFirst(c, c)
    return StructureModuleTransitionLayerJL(linear_1, linear_2, linear_3)
end

function (m::StructureModuleTransitionLayerJL)(s)
    s0 = s
    s = max.(m.linear_1(s), 0f0)
    s = max.(m.linear_2(s), 0f0)
    s = m.linear_3(s)
...
@concrete struct StructureModuleTransitionJL <: Onion.Layer
    layers
    dropout
    layer_norm
end

@layer StructureModuleTransitionJL

function StructureModuleTransitionJL(c::Int, num_layers::Int, dropout_rate::Real)
    layers = [StructureModuleTransitionLayerJL(c) for _ in 1:num_layers]
    drop = SharedDropout(dropout_rate, 3)
    layer_norm = LayerNormFirst(c)
    return StructureModuleTransitionJL(layers, drop, layer_norm)
end

function (m::StructureModuleTransitionJL)(s)
    for l in m.layers
        s = l(s)
    end
    s = m.dropout(s)
...
@concrete struct AngleResnetBlockJL <: Onion.Layer
    linear_1
    linear_2
end

@layer AngleResnetBlockJL

function AngleResnetBlockJL(c_hidden::Int)
    linear_1 = LinearFirst(c_hidden, c_hidden)
    linear_2 = LinearFirst(c_hidden, c_hidden)
    return AngleResnetBlockJL(linear_1, linear_2)
end

function (m::AngleResnetBlockJL)(a)
    s = a
    a = max.(a, 0f0)
    a = m.linear_1(a)
    a = max.(a, 0f0)
...
@concrete struct AngleResnetJL <: Onion.Layer
    linear_in
    linear_initial
    layers
    linear_out
...
function AngleResnetJL(c_in::Int, c_hidden::Int, no_blocks::Int, no_angles::Int, epsilon::Float32)
    linear_in = LinearFirst(c_in, c_hidden)
    linear_initial = LinearFirst(c_in, c_hidden)
    layers = [AngleResnetBlockJL(c_hidden) for _ in 1:no_blocks]
    linear_out = LinearFirst(c_hidden, no_angles * 2)
    return AngleResnetJL(linear_in, linear_initial, layers, linear_out, Float32(epsilon))
end
...
function (m::AngleResnetJL)(s, s_initial)
    s_initial = max.(s_initial, 0f0)
    s_initial = m.linear_initial(s_initial)
    s = max.(s, 0f0)
    s = m.linear_in(s)
...
@concrete struct StructureModuleJLCore <: Onion.Layer
    cfg::StructureModuleConfig
    layer_norm_s
    layer_norm_z
    linear_in
...
@layer StructureModuleJLCore

function _init_residue_constants!(m::StructureModuleJLCore, like)
    default_frames = to_device(restype_rigid_group_default_frame, like, eltype(like))
    group_idx = to_device(restype_atom14_to_rigid_group, like, Int)
    atom_mask = to_device(restype_atom14_mask, like, eltype(like))
    lit_positions = to_device(restype_atom14_rigid_group_positions, like, eltype(like))
...
function StructureModuleJLCore(; cfg::StructureModuleConfig=StructureModuleConfig())
    layer_norm_s = LayerNormFirst(cfg.c_s)
    layer_norm_z = LayerNormFirst(cfg.c_z)
    linear_in = LinearFirst(cfg.c_s, cfg.c_s)
    ipa = InvariantPointAttentionJL(
        cfg.c_s,
        cfg.c_z,
        cfg.c_ipa,
        cfg.no_heads_ipa,
    ...
    transition = StructureModuleTransitionJL(cfg.c_s, cfg.no_transition_layers, cfg.dropout_rate)
    bb_update = BackboneUpdateJL(cfg.c_s)
    angle_resnet = AngleResnetJL(cfg.c_s, cfg.c_resnet, cfg.no_resnet_blocks, cfg.no_angles, cfg.epsilon)

    return StructureModuleJLCore(
        cfg,
        layer_norm_s,
        layer_norm_z,
        linear_in,
...
function (m::StructureModuleJLCore)(evoformer_output_dict, aatype, mask=nothing)
    s = evoformer_output_dict[:single] # (C_s, L, B)
    z = evoformer_output_dict[:pair]   # (C_z, L, L, B)
    ...
    rigids = rigid_identity_jl((size(s, 2), size(s, 3)), s; fmt=:quat)
    ...
    for i in 1:m.cfg.no_blocks
        ...
        rigids = compose_q_update_vec_jl(rigids, m.bb_update(s))

        backb_to_global = RigidJL(RotationJL(rot_mats=get_rot_mats_jl(rigids.rots)), rigids.trans)
        backb_to_global = scale_translation_jl(backb_to_global, m.cfg.trans_scale_factor)

        unnormalized_angles, angles = m.angle_resnet(s, s_initial) # (2, 7, L, B)

        default_frames, group_idx, atom_mask, lit_positions = _init_residue_constants!(m, angles)
        all_frames_to_global = torsion_angles_to_frames_jl(backb_to_global, angles, aatype, default_frames)
        pred_xyz = frames_and_literature_positions_to_atom14_pos_jl(
            all_frames_to_global,
            aatype,
            default_frames,
            group_idx,
        ...
        scaled_rigids = scale_translation_jl(rigids, m.cfg.trans_scale_factor)

        preds = Dict{Symbol,Any}(
            :frames => to_tensor_7_jl(scaled_rigids),
            :sidechain_frames => to_tensor_4x4_jl(all_frames_to_global),
            :unnormalized_angles => unnormalized_angles,
            :angles => angles,
            :positions => pred_xyz,
            :states => s,
